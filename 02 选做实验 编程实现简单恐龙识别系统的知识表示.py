...
def inputRule(count, pre, back,mark):#添加规则   #ask1
    global cat,rules
###########开始1#############
    rules.append(myrule.make_rule(count, pre, back,mark))
    cat += 1

###########结束1#############
    check()
def deleteRule(index):#删除规则  #ask2
###########开始2#############
    global cat,rules
    del rules[index]
    cat -= 1
...
print('添加后：')
inputRule(codeInList[0], codeInList[1], codeInList[2], codeInList[3])
print('删除规则：',codeInList[4])
print('删除后：')
deleteRule(int(codeInList[4])-1)